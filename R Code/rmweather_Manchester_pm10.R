library(remotes)
setwd('C:\\Users\\csy\\OneDrive - Imperial College London\\MSc Tansport\\Dissertation\\Environment\\Coronavirus\\R\\Weather Normalisation\\Dissertation\\Manchester\\Manchester_pm10')
...
pm10 <- read.csv('ManchesterPiccadilly-pm10.csv',head = TRUE)
...
mydata <- left_join(Manche_meteo, pm10, by = "date")

# Save the already prepared dataset
write.csv(mydata,'Manche_meteo+pm10.csv')
...
write.csv(data_Manche_prepared,'Manchester_meteo+pm10_DeNA.csv')
