library(remotes)
setwd('C:\\Users\\csy\\OneDrive - Imperial College London\\MSc Tansport\\Dissertation\\Environment\\Coronavirus\\R\\Weather Normalisation\\Dissertation\\London\\London_O3')
...
o3 <- read.csv('LondonBloomsbury-o3.csv',head = TRUE)
...
mydata <- left_join(london_meteo, o3, by = "date")

# Save the already prepared dataset
write.csv(mydata,'London_meteo+o3.csv')
...
write.csv(data_london_prepared,'London_meteo+o3_DeNA.csv')
...
write.csv(list_normalised[["normalised"]],'london_o3_normalised.csv')
...
comparison <- left_join(list_normalised$normalised, o3, by = "date")
