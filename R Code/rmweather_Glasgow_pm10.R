library(remotes)
setwd('C:\\Users\\csy\\OneDrive - Imperial College London\\MSc Tansport\\Dissertation\\Environment\\Coronavirus\\R\\Weather Normalisation\\Dissertation\\Glasgow\\Glasgow_PM10')
...
pm10 <- read.csv('Glasgow_Townhead-pm10.csv',head = TRUE)
...
mydata <- left_join(Glasg_meteo, pm10, by = "date")

# Save the already prepared dataset
write.csv(mydata,'Glasg_meteo+pm10.csv')
...
write.csv(data_Glasg_prepared,'Glasgow_meteo+pm10_DeNA.csv')
