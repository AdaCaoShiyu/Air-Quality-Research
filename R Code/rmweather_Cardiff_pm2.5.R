library(remotes)
setwd('C:\\Users\\csy\\OneDrive - Imperial College London\\MSc Tansport\\Dissertation\\Environment\\Coronavirus\\R\\Weather Normalisation\\Dissertation\\Cardiff\\Cardiff_PM2.5')
...
pm2.5 <- read.csv('Cardiff_Center-pm2.5.csv',head = TRUE)
...
mydata <- left_join(Cardi_meteo, pm2.5, by = "date")

# Save the already prepared dataset
write.csv(mydata,'Cardi_meteo+pm2.5.csv')
...
write.csv(data_Cardi_prepared,'Cardiff_meteo+pm2.5_DeNA.csv')
