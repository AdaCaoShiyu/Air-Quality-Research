library(remotes)
setwd('C:\\Users\\csy\\OneDrive - Imperial College London\\MSc Tansport\\Dissertation\\Environment\\Coronavirus\\R\\Weather Normalisation\\Dissertation\\Cardiff\\Cardiff_O3')
...
o3 <- read.csv('Cardiff_Center-o3.csv',head = TRUE)
...
mydata <- left_join(Cardi_meteo, o3, by = "date")

# Save the already prepared dataset
write.csv(mydata,'Cardi_meteo+o3.csv')


# Only use data with valid , o3 will become the dependent variable
data_Cardi_prepared <- mydata %>% 
  filter(!is.na(date)) %>%
  filter(!is.na(o3)) %>%
  rename(value = o3) %>% 
  rmw_prepare_data(na.rm = TRUE)
write.csv(data_Cardi_prepared,'Cardiff_meteo+o3_DeNA.csv')
