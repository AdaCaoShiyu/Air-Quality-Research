library(remotes)
setwd('C:\\Users\\csy\\OneDrive - Imperial College London\\MSc Tansport\\Dissertation\\Environment\\Coronavirus\\R\\Weather Normalisation\\Dissertation\\Glasgow\\Glasgow_NOx')
...
nox <- read.csv('Glasgow_Townhead-nox.csv',head = TRUE)
...
mydata <- left_join(Glasg_meteo, nox, by = "date")

# Save the already prepared dataset
write.csv(mydata,'Glasg_meteo+nox.csv')
...
write.csv(data_Glasg_prepared,'Glasgow_meteo+nox_DeNA.csv')
