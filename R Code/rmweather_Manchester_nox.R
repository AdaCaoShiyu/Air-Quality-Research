library(remotes)
setwd('C:\\Users\\csy\\OneDrive - Imperial College London\\MSc Tansport\\Dissertation\\Environment\\Coronavirus\\R\\Weather Normalisation\\Dissertation\\Manchester\\Manche_NOx')
...
nox <- read.csv('ManchesterPiccadilly-nox.csv',head = TRUE)
...
mydata <- left_join(Manche_meteo, nox, by = "date")

# Save the already prepared dataset
write.csv(mydata,'Manche_meteo+nox.csv')
...
write.csv(data_Manche_prepared,'Manchester_meteo+nox_DeNA.csv')
